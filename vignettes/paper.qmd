...
The function takes all arguments needed to run the worker loop (`branin` and `optimizer`) and a `RushWorker` object (`rush`), which manages communication with Redis.
...
The worker loop relies on three principal methods of the Rush class.
...
The `$finish_tasks()` method takes this key along with a list of result values and writes the corresponding results to the database.
...
Tasks that encounter errors can be marked as `"failed"` using the `$fail_tasks()` method; error handling must be implemented explicitly in the worker loop (see @sec-error-handling for details).
...
The worker run on `mirai` daemons which are started with the `mirai::daemons()` function.
...
Further auxiliary information can be passed as lists to the `$push_running_tasks()` and `$finish_tasks()` methods via the `extra` argument.
...
The field-value pairs are written by different methods, e.g. `$push_running_tasks()` writes `xs` and `$finish_tasks()` writes `ys`.
...
For errors that occur during task evaluation, users are responsible for catching them within the worker loop and marking the corresponding task as `"failed"` using the `$fail_tasks()` method:
...
The `$detect_lost_workers()` method identifies such workers and updates their status to `"terminated"`.
...
`mlr3` uses `mirai` to parallelize over the resampling iterations.
...
To quantify this overhead, we benchmark the core functions: `$push_running_tasks()`, `$finish_tasks()`, and `$fetch_finished_tasks()`.

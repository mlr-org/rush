...
Distributed computing provides an effective approach for addressing such large-scale optimization problems.
...
This decentralized architecture allows workers to generate and evaluate tasks independently, thereby avoiding central coordination bottlenecks.
Aiming for minimal overhead per task, rush features robust error handling with automatic detection of lost workers, and an efficient caching mechanism that minimizes database operations.
Rush integrates seamlessly with the `mlr3` ecosystem, powering asynchronous optimization algorithms in the `bbotk` and `mlr3tuning` packages.
We show how `rush` enables the implementation of state-of-the-art optimization algorithms such as asynchronous decentralized Bayesian optimization.
The paper further includes a benchmark evaluating the runtime performance of the core functionality of `rush`.
We demonstrate that `rush` scales efficiently to over 448 workers while achieving higher CPU utilization than traditional approaches.
...
![The architecture of a rush network.](rush_network.png){#fig-rush-network width=100%}
...
![CPU Utilization of CL](cl_cpu_utilization.png)

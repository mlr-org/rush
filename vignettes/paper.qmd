...
![Centralized network](centralized_network.png){#fig-centralized-network}

![Decentralized network](decentralized_network.png){#fig-decentralized-network}
...
![](worker_communication.png){width=100%}
...
xss = lapply(1:25, function(i) list(x1 = lhs_points[i, 1] * 15 - 5, x2 = lhs_points[i, 2] * 15))
```
...
<!-- Split figure and remove legend -->

![](cl_cpu_utilization.png)

Short-running and long-running objective functions.
Each bar represents one BO iteration, with the proposal phase (red) and evaluation phase (blue) shown as stacked components.
...
| Operation               | Median Runtime [ms] | MAD [ms] |
|:------------------------|:--------------------|:---------|
...
Pushing a running task to the database takes a median of 0.5 ms (MAD 0.25 ms), and pushing a result takes approximately the same time.
...
However, as the archive grows beyond 1,000 tasks, caching yields a substantial speedup, as shown in Figure @fig-fetch-finished-tasks.
At 10,000 tasks, fetching without cache takes approximately 84 ms compared to 13 ms with caching.
In both cases, only one new task is retrieved from the database; the remainder is served from the local cache.
The residual increase in runtime with cache enabled is attributable to the cost of binding a new row to the cached `data.table`, which grows with the total number of rows.
In practice, the sub-millisecond overhead of push operations and the low-millisecond overhead of cached fetching are negligible relative to typical task runtimes, which range from seconds to minutes in the BO benchmarks (Table @tbl-benchmarks).

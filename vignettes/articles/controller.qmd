...
A **rush** network can consists of thousands of workers.
To manage the workers, we use the `Rush` controller.
It starts and oversees local and remote workers.
The controller monitors the status of each worker and it can detect and restart failed workers.
Additionally, the controller offers the ability to terminate or kill workers as needed.
The controller reads log messages from workers to help with debugging and monitoring

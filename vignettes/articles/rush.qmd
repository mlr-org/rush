...
*rush* is a package to tackle large-scale problems in a decentralized distributed network.
...
The first examples introduces the main components package worker, worker loop, task and controller.
Further it shows how tasks are created and results are exchanged between workers.
The second example builds a median stopping rule and shows how tasks are fetched from the database.
The third example implements a Bayesian optimization algorithm and introduces the concept of task queues.
...
In our example, the worker draws a random point, evaluates it using the Branin function, and pushes the result back to the Redis database.
...
The main components of a task are the key, state, configuration (`xs`), and result (`ys`).
...
In cases where a task encounters an error, its state is marked as `"failed"` with the `$push_failed()` method (see [Error Handling](error_handling.html) for more details).
Tasks can also be pushed to a queue with the `$push_tasks()` method which sets the state to `"queued"`.
The [Queues](queues.html) article gives more details on the task queue and the different methods to push and pop tasks.

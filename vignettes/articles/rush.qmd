...
`rush` is package to tackle large-scale problems in a distributed and decentralized network of workers.
A rush network consists of multiple workers that communicate over a shared Redis database.
At its core, rush operates through a user-defined worker loop that creates new tasks, evaluates them, and pushes the results to the database
Since the workers communicate over a shared database, they can exchange results and the tasks they are currently working on.
This allows for the implementation of decentralized optimization algorithms that can scale to large problems.
We demonstrate how to use the `rush` package with a simply random search and a more sophisticated random search with a median stopping rule.
For more information on how to start, stop, and observe workers, see the [Rush Controller](rush_controller.html) vignette.
...
We want to optimize the Branin function using a simple random search algorithm.
The algorithm draws random points and evaluates them using the Branin function.
We start by implementing the `worker_loop` that includes the random search algorithm and the communication with the database.
...
We start the optimization process with the median stopping rule and fetch the results.

...
Objects required by the worker loop can be passed as arguments to `$start_local_workers()` / `$start_remote_workers()`.
These arguments are serialized and stored in the Redis database as part of the worker configuration.
Upon initialization, each worker retrieves and unserializes the worker configuration before invoking the worker loop.

::: {.callout-note}

The maximum size of a Redis string is 512 MiB.
If the serialized worker configuration exceeds this limit, Rush will raise an error.
...
Instead of killing the worker processes, it is also possible to send a terminate signal to the worker.
The worker then terminates after it has finished the current task.

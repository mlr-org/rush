...
The `pid` is the process id of the worker process and the `hostname` is the name of the machine.
The `remote` column indicates if the worker is running on a remote machine and the `heartbeat` column shows if a heartbeat process is running.
Heartbeats are explained in the [Error Handling](#error-handling) section.
...
When rush is integrated into a third-party package, the workers are usually started by the package.
The user can call the `rush_plan()` function to set the options for the workers.
We can specify the number and type of workers, and the redux configuration.
...
Global variables are passed by name in the `$start_local_workers()` methods.
...
If the globals of the worker loop function are larger than 512 MiB, `rush` throws an error.
...
The `$detect_lost_workers()` method detects failed workers.
The worker states are updated to `"lost"`.
...
The `lgr_thresholds` argument of `$start_local_workers()` defines the logging level for each logger e.g. `c(rush = "debug")`.

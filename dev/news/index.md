...
- `$fail_tasks()` and `$finish_tasks()` now move a task between states
  atomically with `MULTI`/`EXEC` so that a worker crash mid-move can no
  longer remove a task from the running state without recording it as
  failed or finished.
...
- `$detect_lost_workers()` no longer marks a task as both finished and
  ...
  lost. The running tasks of a lost worker are now read after the worker
  has been flagged as lost, and a running task is moved to the failed
  state with `SMOVE`, which is a no-op when the task has already left
  the running state.
...
- `$worker_script()` no longer logs the Redis password. The logged
  ...
  visibly.
- `RushWorker$new()` now errors when the heartbeat process fails to set
  the heartbeat key within the startup timeout instead of silently
  registering a dead heartbeat. `heartbeat_period` and
  `heartbeat_expire` must now be at least 1 second.
...
- `$fetch_new_tasks()` now correctly tracks seen results by using the
  actual cache size instead of the Redis counter, fixing cases where new
  results could be missed or duplicated.
- `$write_hashes()` now requires all value lists to have the same length
  or length 1 instead of recycling. Length mismatches raise an error
  ([\#87](https://github.com/mlr-org/rush/issues/87)).
- [`rsh()`](https://rush.mlr-org.com/dev/reference/rsh.md) no longer
  accepts `...` (which was ignored).
...
- `$fetch_tasks()` and related methods no longer fail when task

...
* feat: `RushWorker` gains the `$n_queued_available_tasks` field which counts the queued tasks the worker can pop, i.e. the tasks in the shared queue and in the queue of the compute profile the worker runs on.
  The two queues are counted in a single transaction.
...
* fix: `$worker_script()` now validates the `heartbeat_period`, `heartbeat_expire`, `message_log`, and `output_log` arguments instead of interpolating them into the shell command unchecked.

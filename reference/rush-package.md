...
- `rush.max_object_size`: Maximum size in MiB of the serialized worker
  ...
  set in
  [`rush_plan()`](https://rush.mlr-org.com/reference/rush_plan.md), in
  which case the configuration is written to disk instead.
...
- `REDIS_URL`: Connection URL parsed by
  ...
  unset, a default configuration is used.
